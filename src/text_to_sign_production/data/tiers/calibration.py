...
_BINDING_METRICS = BINDING_TIER_METRICS
_DIAGNOSTIC_METRICS = DIAGNOSTIC_TIER_METRICS
...
def build_metric_distribution_records(
    ...
    for split in _split_scopes(metric_bundles):
        ...
        for spec in specs:
            values = tuple(get_metric_value(bundle, spec) for bundle in scoped)
...
def build_confidence_channel_summary_records(
    ...
    return tuple(
        ...
        for record in distribution_records
        if record.family == BindingTierFamily.CONFIDENCE.value
