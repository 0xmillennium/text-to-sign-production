...
from text_to_sign_production.data.metrics.types import ActiveSigningSpanMetrics
...
def _sustained_evidence_mask(frame_evidence: np.ndarray) -> np.ndarray:
    ...
    return sustained & frame_evidence
...
def _landmark_motion_evidence(
    ...
    for index in range(1, num_frames):
        ...
        max_distance = float(np.max(np.linalg.norm(deltas, axis=1)) / normalizer)
...
def _motion_evidence_from_centroids(
    ...
    for index in range(1, num_frames):
        if not (available[index - 1] and available[index]):
            continue
        distance = float(np.linalg.norm(centroids[index] - centroids[index - 1]) / normalizer)
...
def _canvas_diagonal() -> float:
    return math.hypot(CANVAS_WIDTH, CANVAS_HEIGHT)
