...
def load_gates_config(path: Path) -> GatesConfig:
    ...
    min_any_hand = _require_nonnegative_int(
        data["min_any_hand_nonzero_frames"],
        "min_any_hand_nonzero_frames",
    )
    if min_any_hand > min_valid:
        raise ValueError(
            "min_any_hand_nonzero_frames must be <= min_valid_frames, "
            f"got {min_any_hand}>{min_valid}."
        )

    raw_channels = _require_mapping(data["channels"], "channels")
    _require_exact_keys(raw_channels, _CHANNEL_GATE_CONFIG_CHANNELS, "channels")
