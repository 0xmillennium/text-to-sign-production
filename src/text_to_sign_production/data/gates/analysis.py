...
def build_drop_stage_count_records(
    ...
    counter = Counter(decision.drop_stage for decision in decisions if decision.drop_stage is not None)
    return tuple(
...
def build_gate_blocker_summary_records(
    ...
    return tuple(
        ...
        for (stage, reason), count in sorted(counter.items(), key=lambda item: (item[0][0].value, item[0][1]))
    )
