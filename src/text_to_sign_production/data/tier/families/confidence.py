...
def compute_confidence_metrics(input: QualityMetricBuildInput) -> ConfidenceMetrics:
    ...
    return ConfidenceMetrics(
        active_span_body_mean_confidence=_mean_active_confidence(
            input.sample.pose.body_xyc[..., 2], active
        ),
        active_span_hand_mean_confidence=_mean_active_confidence(hand_conf, active),
        active_span_face_mean_confidence=_mean_active_confidence(
            input.sample.pose.face_xyc[..., 2], active
        ),
    )


def _mean_active_confidence(confidences: np.ndarray, active: np.ndarray) -> float:
