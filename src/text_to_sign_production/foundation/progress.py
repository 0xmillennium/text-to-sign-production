...
class ItemProgress:
    ...
    _postfix_text: str = field(init=False, default="", repr=False)
    ...
    def __post_init__(self) -> None:
        self._bar = progress_bar(
            ...
            bar_format=_progress_bar_format(
                total=self.total,
                postfix_text=self._postfix_text,
            ),
        )
    ...
    def _refresh_bar_format(self) -> None:
        if self._bar is None:
            return
        self._bar.bar_format = _progress_bar_format(
            total=self.total,
            postfix_text=self._postfix_text,
        )
        self._bar.refresh()
...
def _progress_bar_format(total: int | None, postfix_text: str) -> str:
    context = _progress_bar_context(postfix_text)
    if total is None:
        return (
            "{desc}: {n_fmt} "
            f"[elapsed: {{elapsed}} | speed: {{rate_fmt}}{context}]"
        )
    return (
        "{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} "
        f"[elapsed: {{elapsed}} | remaining: {{remaining}} | "
        f"speed: {{rate_fmt}}{context}]"
    )


def _progress_bar_context(postfix_text: str) -> str:
    if not postfix_text:
        return ""
    return " | " + postfix_text.replace("{", "{{").replace("}", "}}")
...
def _render_progress_fields(fields: dict[str, str]) -> str:
