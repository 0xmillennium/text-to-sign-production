...
class GateWorkflowError(Exception):
    pass


class GateWorkflowInputError(GateWorkflowError):
    pass


class GateWorkflowInvariantError(GateWorkflowError):
    pass


@dataclass(frozen=True, slots=True)
class GateWorkflowConfig:
    project_root: Path
...
def _coerce_path(value: object) -> Path:
    if value is None:
        raise GateWorkflowInputError("path values must not be None")
    if isinstance(value, str):
        raw_path = value
    elif isinstance(value, os.PathLike):
        raw_path = os.fspath(cast(os.PathLike[str], value))
    else:
        raise GateWorkflowInputError("path values must be strings or path-like objects")
    if not isinstance(raw_path, str):
        raise GateWorkflowInputError("path values must be text paths")
    if not raw_path.strip():
        raise GateWorkflowInputError("path values must not be blank")
...
def _coerce_relative_path(value: object) -> Path:
    ...
    if path.is_absolute():
        raise GateWorkflowInputError("gates_config_relpath must be relative")
    return path
...
def _coerce_splits(value: object) -> tuple[str, ...]:
    if isinstance(value, str):
        ...
        raise GateWorkflowInputError("splits must be an iterable of split names")
    splits = tuple(_coerce_split_item(split) for split in raw_splits)
    for split in splits:
        _validate_non_empty_split(split)
    if len(set(splits)) != len(splits):
        raise GateWorkflowInputError("splits must not contain duplicates")
    return splits


def _coerce_split_item(value: object) -> str:
    if not isinstance(value, str):
        raise GateWorkflowInputError("split names must be strings")
    return value.strip()
