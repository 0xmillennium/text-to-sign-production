...
from text_to_sign_production.legacy_data._shared.analysis import numeric_summary
...
def build_sample_processing_summary_records(
    ...
    *,
    ...
    counts = {(record.split, record.status): record.count for record in split_status_counts}
    return tuple(
        SampleProcessingSummaryRecord(
            split=split,
            passed_count=counts.get((split, SampleStatus.PASSED), 0),
            dropped_count=counts.get((split, SampleStatus.DROPPED), 0),
            total_count=sum(counts.get((split, status), 0) for status in SampleStatus),
        )
...
def build_sample_dropped_materialization_summary_records(
    ...
    *,
    ...
    dropped_entries = tuple(entry for entry in entries if isinstance(entry, DroppedManifestEntry))
    ordered_splits = tuple(
        sorted(
            {entry.split for entry in dropped_entries} | {entry_split for entry_split in splits},
            key=lambda item: item.value,
        )
    )
    return tuple(
        _dropped_materialization_summary_record(split, dropped_entries) for split in ordered_splits
    )
...
def _dropped_materialization_summary_record(
    ...
    outcome_counter = Counter(
        entry.materialization.debug_materialization_outcome for entry in split_entries
    )
    eligible_count = sum(
        1 for entry in split_entries if entry.materialization.debug_materialization_eligible
    )
    attempted_count = sum(
        1 for entry in split_entries if entry.materialization.debug_materialization_attempted
    )
    payload_exists_count = sum(1 for entry in split_entries if entry.materialization.payload_exists)
    archive_publishable_count = sum(
        1 for entry in split_entries if entry.materialization.archive_publishable
    )
