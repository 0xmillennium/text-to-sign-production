...
from text_to_sign_production.legacy_data.samples._shared.parsing import (
    bool_from_record,
...
def manifest_entry_from_record(record: Mapping[str, Any]) -> ManifestEntry:
    ...
    if status is SampleStatus.DROPPED:
        require_record_keys(
            record, REQUIRED_DROPPED_MANIFEST_KEYS, surface="Dropped manifest entry"
        )
        ...
        with manifest_path.open(encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                if not isinstance(raw, Mapping):
                    raise TypeError(
                        f"Manifest {manifest_path} line {line_number} must contain a JSON object."
                    )
                if any(not isinstance(key, str) for key in raw):
                    raise TypeError(
                        f"Manifest {manifest_path} line {line_number} must contain string keys."
                    )
