...
from text_to_sign_production.legacy_data._shared.validate import shape_tuple
from text_to_sign_production.legacy_data.pose.schema import OPENPOSE_CHANNEL_SPECS
from text_to_sign_production.legacy_data.pose.types import PoseBuildOutput, PoseValidationIssue
...
def validate_pose_build(output: PoseBuildOutput) -> list[PoseValidationIssue]:
    ...
    for name, channel in channels.items():
        ...
        if (coords_shape is not None and coords_shape[0] != expected_frames) or (
            conf_shape is not None and conf_shape[0] != expected_frames
        ):
