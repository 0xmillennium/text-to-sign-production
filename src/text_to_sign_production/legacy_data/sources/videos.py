...
from text_to_sign_production.legacy_data.sources.types import VideoMetadataFacts
...
def read_video_metadata(path: Path) -> VideoMetadataFacts:
    """Read lightweight width, height, and FPS metadata from an MP4."""

    if not path.exists():
        return VideoMetadataFacts(width=None, height=None, fps=None, error="video_file_missing")
    ...
    try:
        with path.open("rb") as handle:
            ...
            if trak is None:
                return VideoMetadataFacts(
                    width=None, height=None, fps=None, error="video_track_missing"
                )
            ...
            return VideoMetadataFacts(width=width, height=height, fps=round(fps, 6), error=None)
