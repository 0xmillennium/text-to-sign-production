...
- [ADR-0013: Define the Post-Dataset-Build Research Roadmap](adr-0013-define-post-dataset-build-research-roadmap.md): freeze the planned baseline, evaluation, and contribution sequence as historical roadmap context.
...
- [ADR-0018: Roadmap as Normative Sprint Contract](adr-0018-roadmap-as-normative-sprint-contract.md): make the roadmap the ideal sprint contract while keeping implementation status in separate audits and summaries.
- [ADR-0019: Split Notebook Architecture as Target Workflow](adr-0019-split-notebook-architecture-as-target-workflow.md): define dedicated thin-driver notebook surfaces as the target workflow during transition from the current single Colab.
- [ADR-0020: Published Model Artifacts as Downstream Interface](adr-0020-published-model-artifacts-as-downstream-interface.md): make published model artifacts the stable downstream interface instead of local checkpoints or runtime evidence.
- [ADR-0021: Four-Model 2x2 Comparison Architecture](adr-0021-four-model-2x2-comparison-architecture.md): organize thesis comparison around `M0`, `M1`, `M2`, and `M3` while keeping C1 and C2 as contribution roles.
- [ADR-0022: Standard Testing Surfaces Before Contribution Implementation](adr-0022-standard-testing-surfaces-before-contribution-implementation.md): require released baseline and reusable testing surfaces before full C1/C2 model implementation.
